...
function joinHandler(player)
	...
	if getOption('welcometextonstart') then
		outputChatBox('Welcome to MOGA drift/Freeroam', player, 143, 0, 255)
		outputChatBox('Press F1 ro show/hide your MotorSport panel', player, 143, 0, 255)
	end
end
...
function giveMeVehicles(vehicles)
	...
	-- Edited from here!
	local px, py, pz, protx, proty, protz -- Define the data
	local radius = 3
	local playerVehicle = getPedOccupiedVehicle(source)
	if playerVehicle and isElement(playerVehicle) then -- If there player was in a vehicle
		px, py, pz = getElementPosition(playerVehicle) -- Get the position of the vehicle
		protx, proty, protz = getVehicleRotation(playerVehicle) -- Get the rotation of the vehicle
	else -- If the player was on foot
		px, py, pz = getElementPosition(source) -- Get the position of the player
		protz = getPedRotation(source) -- Get the Z rotation of the player
		protx, proty = 0, 0 -- Define the X and Y rotation as 0, since 'getPedRotation' function would only return the Z rotation
	end
	--[[ Things below are not neccessary because the new function would use enhanced data
	...
	for i,vehID in ipairs(vehicles) do
		...
		elseif not table.find(getOption('vehicles.disallowed'), vehID) then 
			if playerVehicle and isElement(playerVehicle) then -- If the player was in a vehicle
				setElementModel (playerVehicle, vehID) -- Change the model of the current vehicle
				vehicle = playerVehicle -- set 'vehicle' data into 'playerVehicle' for further function
			else -- If the player was on foot
				vehicle = createVehicle(vehID, px, py, pz, protx, proty, protz) -- Create a vehicle with the desired ID, and follow the position and rotation of the player
				if (not isElement(vehicle)) then return end -- If it couldn't create the vehicle, skip the whole function
				warpPedIntoVehicle (source, vehicle) -- Warp the player to the vehicle right away
				if #vehicleList >= getOption('vehicles.maxperplayer') then -- If the player passed the vehicle limit
					unloadVehicle(vehicleList[1]) -- Remove the first vehicle on the list (in this case the old one, based on your desired limit by the way, call me on Skype for further information about this)
				end
				table.insert(vehicleList, vehicle) -- Add the vehicle to 'vehicleList' table, so it would be possible for the function to check old car to remove with the function above
				g_VehicleData[vehicle] = { creator = source, timers = {} } -- Add neccessary data to the table
			end
			setElementInterior(vehicle, getElementInterior(source)) -- Set the Interior of the vehicle into the same Interior as the player
			setElementDimension(vehicle, getElementDimension(source)) -- Set the Dimension of the vehicle into the same Dimension as the player
			if not g_Trailers[vehID] then -- If the desired Vehicle ID isn't trailer
				if getOption('vehicles.idleexplode') then -- If you want the system to blow/remove the vehicle (freeroam setting)
					g_VehicleData[vehicle].timers.fire = setTimer(commitArsonOnVehicle, getOption('vehicles.maxidletime'), 1, vehicle) -- Add neccessary data to the table about the time limit
				end
				g_VehicleData[vehicle].timers.destroy = setTimer(unloadVehicle, getOption('vehicles.maxidletime') + (getOption('vehicles.idleexplode') and 10000 or 0), 1, vehicle) -- Call the function to blow/destroy the vehicle based on the time limit from function above
			end
			-- Until here!
		else
