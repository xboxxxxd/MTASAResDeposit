...
---------------------------
function toggleBikeFallOff()
	if guiCheckBoxGetSelected(getControl(wndMain,'bikefalloff'),true) then
		setPedCanBeKnockedOffBike(g_Me,false)
		guiCheckBoxSetSelected(getControl(wndMain,'bikefalloff'),true)
		
	else
		setPedCanBeKnockedOffBike(g_Me,true)
		guiCheckBoxSetSelected(getControl(wndMain,'bikefalloff'),false)
	end
end
...
function flipVehicle()
	local vehicle = getPedOccupiedVehicle(g_Me)
	if vehicle then
	if (getVehicleController(vehicle) == g_Me) then
...
function namecolor()
	triggerEvent("openNCWindow", getRootElement())
end

function openTimeSettings()
	if (guiGetVisible(timeSettingsWindow) == false) then
	guiSetVisible(timeSettingsWindow,true)
	showCursor(true)
	end
end
...
wndMain = {
	'wnd',
	text = '*Moga MotorSport Server 1.4*',
	x = 10,
	y = 180,
	width = 280,
	controls = {
		{'lbl', text='Welcome To Dj Freeroam'},
		{'br'},
		{'btn', id='Character', window=wndSkin},
		{'btn', id='Animation', window=wndAnim},
		{'btn', id='Body Type', window=wndStats},
		{'btn', id='CJ Clothes', window=wndClothes},
		...
		{'br'},	
		{'br'},
		{'chk', id='bikefalloff', text='Stay On Bike', onclick=toggleBikeFallOff},
		{'br'},
		{'btn', id='Graphics', window=wndPaintjob},
		{'btn', id='Paint', onclick=openColorPicker},
		{'btn', id='Customize', window=wndUpgrades},
		{'btn', id='playergrav', text='grav', window=wndGravity},
		{'br'},
		{'btn', id='Repair', onclick=repairVehicle},
		{'btn', id='Flip', onclick=flipVehicle},
		{'lbl', text='Vehicle Name:'},
                                    {'lbl', id='curvehicle'},
		{'br'},
		{'btn', id='createvehicle', window=wndCreateVehicle, text='Vehicle List'},
                                    {'br'},
		{'chk', id='lightson', text='Lights on', onclick=forceLightsOn},
		{'chk', id='lightsoff', text='Lights off', onclick=forceLightsOff},
                                    {'lbl', text='Made by: Leito'},
	},
...
timeSettingsWindow = guiCreateWindow(0.4356,0.3044,0.1475,0.2778,"Time Settings",true)
guiSetVisible(timeSettingsWindow,false)
frozenbut = guiCreateRadioButton(0.0805,0.124,0.3432,0.092,"Frozen",true,timeSettingsWindow)
localtimebut = guiCreateRadioButton(0.0805,0.24,0.3686,0.088,"Local time",true,timeSettingsWindow)
gtatimebut = guiCreateRadioButton(0.0805,0.356,0.3771,0.076,"GTA time",true,timeSettingsWindow)
closebut = guiCreateButton(0.7076,0.868,0.2373,0.088,"Close",true,timeSettingsWindow)

function closeTimeGUI()
	if source == closebut then
		if (guiGetVisible(timeSettingsWindow) == true) then
		guiSetVisible(timeSettingsWindow,false)
		showCursor(false)
		end
	end
end
addEventHandler("onClientGUIClick", getResourceRootElement(getThisResource()), closeTimeGUI)

function timeStuff()
	if source == frozenbut then	
	setMinuteDuration(99999999)
elseif source == localtimebut then
	syncTime()
	setMinuteDuration(60000)
	timer = setTimer(syncTime, 500, 0)
elseif source == gtatimebut then
	if not isTimer(timer) then
	setMinuteDuration(1000)
elseif isTimer(timer) then killTimer(timer)
		local hour1, minute1 = getTime()
		setTime( hour1, minute1 )
		setMinuteDuration(1000)
		end
	end
end
addEventHandler("onClientGUIClick", getResourceRootElement(getThisResource()), timeStuff)
